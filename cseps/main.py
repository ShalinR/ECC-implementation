...
def action_create_tender():
    ...
    # Deadline: accept minutes from now
    deadline_mins_str = prompt("Deadline (minutes from now)", "1")
    try:
        deadline_mins = int(deadline_mins_str)
        deadline_dt = datetime.now(timezone.utc) + timedelta(minutes=deadline_mins)
        deadline_iso = deadline_dt.isoformat()
    except ValueError:
        error(f"Invalid deadline: '{deadline_mins_str}' is not a valid number.")
        return
    ...
    try:
        result = admin_mod.create_tender(title, deadline_iso, num_eval, thresh, admin_name, admin_pwd)
        t = result["tender"]
        ...
        print(f"  {Color.RED}{Color.BOLD}⚠  Shares are saved to files for secure delivery; only truncated previews are shown.{Color.RESET}\n")
        # Ensure shares directory exists
        shares_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), "data", "keys", "shares")
        os.makedirs(shares_dir, exist_ok=True)

        for i, share in enumerate(result["shares"]):
            # Save full share to a file for secure out-of-band delivery to evaluator
            share_fname = f"{t['tender_id']}_share_{i+1}.share"
            share_path = os.path.join(shares_dir, share_fname)
            with open(share_path, "w", encoding="utf-8") as sf:
                sf.write(share)

            # Show only truncated preview to the admin
            preview = share[:20] + "..." if len(share) > 20 else share
            print(f"    Share #{i+1}: {Color.GREEN}{preview}{Color.RESET}  (saved: {share_path})")
...
def action_open_bids():
    ...
    try:
        result = admin_mod.open_bids(tender_id, shares, admin_name, admin_pwd)
        results = result["bids"]
        ...
        if winner:
            section("WINNER")
            print(f"  {Color.BOLD}{Color.GREEN}Bidder: {winner['bidder']}{Color.RESET}")
            print(f"  {Color.BOLD}{Color.GREEN}Winning Amount: ${winner['bid_data'].get('bid_amount', 'N/A')}{Color.RESET}")
            print(f"  Details: {winner['bid_data'].get('details', 'N/A')}")
        else:
            warn("No valid bids to declare a winner.")
...
def action_audit_log():
    section("Admin: Audit Report")
    admin_name = prompt("Admin username")
    admin_pwd = prompt_password("Admin password")
    try:
        admin_mod.authenticate_admin(admin_name, admin_pwd)
    except Exception as e:
...
def action_demo_workflow():
    ...
    result = admin_mod.open_bids(t["tender_id"], [shares[0], shares[1]], "admin_demo", "adminpass")
    results = result["bids"]
    winner = result["winner"]
    ...
    if winner:
        print(f"\n  {Color.BOLD}{Color.GREEN}WINNER: {winner['bidder']} (${winner['bid_data'].get('bid_amount', '?')}){Color.RESET}")
