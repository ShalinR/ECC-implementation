...
def verify_bid_file(bid_filepath: str) -> dict:
    ...
    # Note: `current_hash` is already a hex digest string. Do not re-hash the hex string.
    submissions = ledger.get_entries("BID_SUBMITTED")
    hash_in_ledger = any(e.get("data_hash") == current_hash for e in submissions)
...
def generate_audit_report() -> str:
    ...
    lines.append(f"  Chain integrity: {'VALID [OK]' if is_valid else 'COMPROMISED [FAIL]'}")
    if errors:
        for err in errors:
