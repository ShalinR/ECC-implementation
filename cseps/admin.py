...
def open_bids(tender_id: str, shares_b64: list[str], admin_name: str = None, admin_password: str = None) -> list[dict]:
    ...
    # Determine winner (lowest valid bid wins)
    winner = None
    if results:
        valid_bids = [
            r for r in results
            if r["decrypted"] and r["signature_valid"] and r["hash_valid"]
            and isinstance(r["bid_data"], dict)
            and "bid_amount" in r["bid_data"]
        ]
        if valid_bids:
            # Sort by bid_amount (highest wins)
            sorted_bids = sorted(valid_bids, key=lambda x: float(x["bid_data"].get("bid_amount", 0)), reverse=True)
            winner = sorted_bids[0]
            
            # Log winner to ledger
            ledger.add_entry(
                "WINNER_DECLARED",
                json.dumps({
                    "tender_id": tender_id,
                    "winner": winner["bidder"],
                    "winning_amount": winner["bid_data"].get("bid_amount"),
    ...
    return {"bids": results, "winner": winner}
